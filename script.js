...
const YOUR_EMAIL = 'your.email@gmail.com';  // 👈 CHANGE THIS TO YOUR EMAIL

// Email integration function
function sendEmail(emotion, message) {
    const timestamp = new Date().toLocaleString();
    const subject = `💕 ${emotion.charAt(0).toUpperCase() + emotion.slice(1)} feelings from Nehu`;
    const body = `Hi my love! 💕

Nehu is feeling: ${emotion.toUpperCase()}
Time: ${timestamp}
...
---
Sent from Nehu Feelings of the Day 🌸
This is an automated message from your girlfriend's feelings portal.`;
...
if (document.getElementById('angryForm')) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (textarea.value.trim()) {
            // Send email first
            sendEmail('angry', textarea.value);
...
if (document.getElementById('overthinkingForm')) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (textarea.value.trim()) {
            // Send email first
            sendEmail('overthinking', textarea.value);
